...
export default class QueueService {
  ...
  async EnqueueEmail(args: SQSSendEmailArgs<any>) {
    const command = new SendMessageCommand({
      ...
      MessageBody: JSON.stringify({
        type: SQSMessageType.EMAIL,
        payload: args,
      }),
    });
    const result = await this._sqsClient.send(command);
    Logger.Info(
      `Email enqueued: ${args.template} | ID: ${result.MessageId}`,
      '[QUEUE]',
    );
  }
  ...
  async EnqueueNotification(args: SQSSendNotificationArgs<any>) {
    const command = new SendMessageCommand({
      QueueUrl: AppConfig.AWS.QUEUE_URL,
      MessageBody: JSON.stringify({
        type: SQSMessageType.NOTIFICATION,
        payload: args,
      }),
    });
    const result = await this._sqsClient.send(command);
    Logger.Info(
      `Notification enqueued: ${args.type} | ID: ${result.MessageId}`,
      '[QUEUE]',
    );
  }
  ...
  async EnqueueCertificate(args: SQSGenerateCertificateArgs) {
    const command = new SendMessageCommand({
      QueueUrl: AppConfig.AWS.QUEUE_URL,
      MessageBody: JSON.stringify({
        type: SQSMessageType.CERTIFICATE,
        payload: args,
      }),
    });
    const result = await this._sqsClient.send(command);
